...
//    mixedradix_endo_gray M(n1, rq);
    ...
    do
    {
        ...
        if ( rq )  rfact2perm_rev(a, n, p);
        else       ffact2perm_rev(a, n, p);
        print_perm("    ", p, n, dfz);
...
//        print_cycles("  ", ip, n);
        ...
        if ( rq ) perm2rfact_rev(ip, n, ia);
        else      perm2ffact_rev(ip, n, ia);
        print_mixedradix("    ", ia, n1, dfz);

#if 1  // Testing that Xfact2perm() is inverse of perm2Xfact():
        if ( rq )  perm2rfact_rev(p, n, t);
        else       perm2ffact_rev(p, n, t);
...
/// MyRelDir: "demo/comb"
/// makefile-dir: "../../"
/// make-target: "1demo DSRC=demo/comb/fact2perm-rev-demo.cc"
/// make-target2: "1demo DSRC=demo/comb/fact2perm-rev-demo.cc DEMOFLAGS=-DTIMING"
/// End:
